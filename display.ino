...
// just to make sure nothing got left behind
 matrix.clear();
  matrix2.clear();

 // below code is to do the dim bright stuff
...
int h = 0;
 ...
 WiFiClient client;
   ...
   // if (http.begin(client, "http://192.168.81.157:8080/getdata")) {  // HTTP

  // String urlgetdata = "http://" + String(settings.eyedro1) + ":8080/getdata";

       if (http.begin(client, "http://" + String(settings.eyedro1) + ":8080/getdata")) {  // HTTP
      ...
      //  Serial.printf("[HTTP] GET... code: %d\n", httpCode);
        ...
        if (httpCode == HTTP_CODE_OK || httpCode == HTTP_CODE_MOVED_PERMANENTLY) {
         ...
         const size_t capacity = JSON_ARRAY_SIZE(2) + 2*JSON_ARRAY_SIZE(4) + JSON_OBJECT_SIZE(1) + 10;
          DynamicJsonDocument doc(capacity);
          //deserializeJson(doc, json);

       // const char* json = "{\"data\":[[58,12290,32000,22000],[-920,12287,62000,-70000]]}";
      //deserializeJson(doc, json);
         deserializeJson(doc, http.getString());
        //  JsonObject& root = deserializeJson(doc, http.getString());

          
          JsonArray data_0 = doc["data"][0];
          int data_0_0 = data_0[0]; // 58
          int data_0_1 = data_0[1]; // 12290
          long data_0_2 = data_0[2]; // 32000
          long data_0_3 = data_0[3]; // 22000
          
          JsonArray data_1 = doc["data"][1];
          int data_1_0 = data_1[0]; // -920
          int data_1_1 = data_1[1]; // 12287
          long data_1_2 = data_1[2]; // 62000
          long data_1_3 = data_1[3]; // -70000


            // ok lets add the kw together to get our total

          long totalKwh = data_0_3 + data_1_3;

          // testing
        // totalKwh = -9923;

          // blank out the left side

          if ((totalKwh < 10000) && (totalKwh > -1000)) {
              matrix.clear();
              matrix2.print(totalKwh);
          } else {
        

              // handle the minus sign that overflows 
              if ((totalKwh < 0) && (totalKwh > -9999)) {
                    // the minus has to go onto the left side
                     matrix.writeDigitRaw(4, dash);
              }  else {

              
              matrix.print(totalKwh / 10000);

              }

            
            matrix2.print(abs(totalKwh) % 10000);
          }
          
          //matrix.print(data_0_3, DEC);
